...
const LOG_FILE = path.join(__dirname, '../teamheals_monitor.log');
...
async function fetchModInfo(modConfig) {
  try {
    ...
    // 保存HTML到文件以便调试
    fs.writeFileSync('last_response.html', response.data, 'utf8');
    ...
    try {
      ...
      if (error.response && (error.response.status === 302 || error.response.status === 301)) {
        ...
        try {
          ...
          try {
            execSync('which wget');
            command = `wget -O "${filePath}" --user-agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36" --header="Referer: https://thunderstore.io/" "${url}"`;
          } catch (e) {
            try {
              execSync('which curl');
              command = `curl -L -o "${filePath}" -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36" -e "https://thunderstore.io/" "${url}"`;
            } catch (e) {
              throw new Error('未找到wget或curl工具');
            }
          }
          ...
          log(`使用shell命令下载失败: ${shellError.message}`);
          reject(shellError);
        }
      } else {
        reject(error);
...
async function checkAndUpdateMod(modConfig) {
  try {
    ...
    // 首先检查版本号是否变更
    if (modInfo.version !== currentModInfo.version) {
      isUpdated = true;
    ...
    if (isUpdated) {
      ...
      // 检查页面文件是否存在
      let pageUpdateResult = false;
      if (fs.existsSync(modConfig.pageFile)) {
        // 更新网站页面
        pageUpdateResult = updateModPage(modConfig, modInfo);
        if (pageUpdateResult) {
          log(`已更新网站页面: ${modConfig.pageFile}`);
        }
      } else {
        log(`页面文件不存在，跳过更新页面: ${modConfig.pageFile}`);
        // 仍然视为成功，因为我们已经下载了文件
        pageUpdateResult = true;
      }
...
async function main() {
  ...
  for (const modConfig of MOD_CONFIGS) {
    await checkAndUpdateMod(modConfig);
  }
